...
void worker(int start, int end, int n, i64 &res1, i64 &res2) {
    i64 local1 = 0, local2 = 0;
    for (int a = start; a < end; a++) {
        for (int b = 1; b <= n; b++) {
            local1 += EuclideanCount(a, b);
            local2 += EuclideanCount_v2(a, b);
        }
    }
    res1 = local1;
    res2 = local2;
...
int main() {
    ...
    vector<i64> partial1(num_threads, 0), partial2(num_threads, 0); //used to store the partial results from each threads runtime, as the original vector pair was storing 1 million + objects in sys memory

    int chunk = (n - 1 ) / num_threads; // seperates the range for a ot for mutliple threads to use

    for (int t = 0; t < num_threads; t++) { //loop creates and launces thread for each chunk of work
        int start = t * chunk + 1;
        int end = (t == num_threads - 1) ? n : (t + 1) * chunk + 1; //number of values each thread has to run
        threads.emplace_back(worker, start, end, n, ref(partial1[t]), ref(partial2[t])); // launches th threads to actuially do the work
    }
